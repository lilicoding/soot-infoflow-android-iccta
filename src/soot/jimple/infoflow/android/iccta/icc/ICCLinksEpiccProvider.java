...
import soot.jimple.infoflow.android.iccta.db.ConnectionProviderManager;
...
public class ICCLinksEpiccProvider implements IICCLinksProvider 
{
	...
	public static final String DB_NAME = "cc";
	static
	{
	...
	public List<Integer> fetchInstructions(String SMString, String[] appNames)
	{
		...
		try 
		{
			instructions = (List<Integer>) adapter.executeQuery(sql, value, DB_NAME);
	...
	*/
	...
	public List<ICCLink> fetchICCLinks(String[] appNames) 
	{
		...
		try 
		{
			links = (List<ICCLink>) adapter.executeQuery(sql, paramValues, DB_NAME);
		} 
		catch (SQLException e) 
		{
			e.printStackTrace();
	...
	public List<IACLink> fetchIACLinks()
	{
		...
		try
		{
			links = (List<IACLink>) adapter.executeQuery(sql, null, DB_NAME);
		}
		catch (SQLException e)
		{
			e.printStackTrace();
